...
-- ======================
-- Base entity tables (ordered so FK targets exist before children)
-- ======================
...
CREATE TABLE seats (
    ...
    CONSTRAINT uq_seat_venue_section_row_seat UNIQUE (venue_id, `section`, row_label, seat_number),
...
CREATE TABLE events (
    ...
    CONSTRAINT chk_event_status CHECK (status IN ('scheduled','postponed','cancelled','finished')),
    CONSTRAINT chk_event_times CHECK (end_time IS NULL OR end_time >= start_time)
) ENGINE=InnoDB;

-- ======================
-- Events ISA (concert_events needs artists table to exist -> artists created above)
-- ======================
...
CREATE TABLE conference_events (
    ...
    PRIMARY KEY (event_id),
    CONSTRAINT fk_conferences_event FOREIGN KEY (event_id) REFERENCES events(event_id) ON DELETE CASCADE
) ENGINE=InnoDB;
...
CREATE TABLE regular_tickets (
    ...
    -- Note: MariaDB may ignore CHECK constraints on datetime comparisons on some versions
) ENGINE=InnoDB;
...
CREATE TABLE purchases (
    ...
    CONSTRAINT chk_purchase_status CHECK (status IN ('pending','completed','refunded','cancelled')),
...
CREATE TABLE payments (
    ...
    CONSTRAINT chk_payment_status CHECK (status IN ('ok','failed','pending','refunded')),
